...
function confirmCancelOp(e,id,_pmyself,go) {
  e.stopPropagation();
  e.preventDefault();
  function callback(){
    RequireUtils.baseRequire('/order/confirm-cancel',{id:id},function (data) {
      if(data.code==1) {
        message.success(data.msg);
        go.bind(_pmyself)() ;
      }else{
...
function comfirmAccount(order_id,_pmyself,go){
...
function getSaleButton(record,_pmyself,go) {
  ...
  const confirmOrder = <span className={'my-custom-center'} style={{marginBottom:'3px',marginRight:'5px'}}>
  ...
  const confirmSum =  <span className={'my-custom-center'}  style={{marginRight:'5px'}}>
                        <Button type="primary" onClick={()=>comfirmAccount.bind(this)(record.id,_pmyself,go)}>确认结算</Button>
                      </span> ;
  ...
  const confirmCancel =  <span className={'my-custom-center'} style={{marginRight:'5px'}}  onClick={(e)=>confirmCancelOp.bind(this)(e,record.id,_pmyself,go)}>
